...
setInterval(() => {
  http.get("`http://arabgames12.glitch.me/`");
}, 280000);
...
client.on("message", message => {
  ...
  if (
    ...
    message.channel.send("**لديك __15__ ثانيه لحل هذا اللغز**").then(msg => {
      const w = ["./img/w1.png"]; //الخلفيه
      let Image = Canvas.Image,
        canvas = new Canvas(400, 150),
        ctx = canvas.getContext("2d");
...
client.on("message", function(message) {
  if (
    message.content.startsWith(prefix + "حجر") ||
...
client.on("message", luxy => {
  ...
  if (luxy.content === "لعب") {
    let embed = new Discord.RichEmbed()
      ...
      .setColor("RED")
      .setDescription(
        `**~~=~~ Bot Orders | 'قائمة الألعاب' ~~=~~
------------------------------------------
يجب وضع علامة (*) قبل اللعبة
...
---------------------------------**`
      ...
      .setFooter("by @Glitch Squirtle #7647");
  ...
  switch (toxicc.content.split(" ")[0]) {
    case prefix + "سيرفر":
      var embed = new Discord.RichEmbed()
        ...
        .addField("Name: ", toxicc.guild.name, true)
        .addField("ID: ", toxicc.guild.id, true)
        .addField("Ownership: ", toxicc.guild.owner, true)
        .addField("Region: ", toxicc.guild.region, true)
        .addField(
          "Members: ",
          toxicc.guild.members.filter(member => !member.user.bot).size,
          true
        )
        .addField(
          "Bots: ",
        ...
        .addField("Channels: ", toxicc.guild.channels.size, true)
        .addField("Roles: ", toxicc.guild.roles.size, true);
